...
def admin_add_entity():
    ...
    entity = request.path.split("/")[-2]  # "doctor" or "patient"
    if request.method == "POST":
        ...
        db.session.commit()
        flash(f"{entity.title()[:-1]} added successfully!", "success")
        return redirect(url_for("manage_" + entity + "s"))
